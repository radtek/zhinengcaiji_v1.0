...
列,行,VIP用户语音呼叫次数,VIP用户数据呼叫次数,VIP用户语音呼叫掉话次数,VIP用户数据呼叫掉话次数,VIP用户短信呼叫次数,VIP用户短信呼叫掉话次数,VIP用户呼叫EC/IO平均数
...
<#if rs??><#assign  rowcolumn=rs?split(',')> 
          <#assign len=rowcolumn?size-1 />
					<#list 0..len as d>${rowcolumn[d]}<#if d != len>,</#if></#list>,${onearray[o.indexs.d2.VOICE_CALL_NUM]},${onearray[o.indexs.d2.DATA_CALL_NUM]},${onearray[o.indexs.d2.SMS_CALL_NUM]},${onearray[o.indexs.d2.VOICE_DROP_NUM]},${onearray[o.indexs.d2.DATA_DROP_NUM]},${onearray[o.indexs.d2.SMS_DROP_NUM]},${onearray[o.indexs.d2.ACCESSPNSTRAVG]}
</#if></#if></#list>