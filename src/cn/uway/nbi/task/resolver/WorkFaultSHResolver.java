...
/**
 * 
 * 上海工单数据，将工单数据组装成xml之后，调用设计院，中通软的webservice 接口 将消息发送给对方， 并记录对方反馈过来的日志状态。
 * 
 * 
 */
@IDMember(id = 112)
public class WorkFaultSHResolver extends DBResolver {

	private static final Logger LOG = LoggerFactory.getLogger(WorkFaultSHResolver.class);
		...
		for (MappingInfo info : mappingInfos) {
			try {
				...
				for (int i = 0; i < info.sqls.size(); i++) {
					...
					sb.append("<INTERFACE_NAME> FaultDealresultToKB</INTERFACE_NAME>").append("\n");

					int count = 0;
					while (rs.next()) {

						sb.append("<REQUESTDATA>").append("\n");
						sb.append("<REQUESTITEM>").append("\n");

						String WORKORDER_NO = rs.getString("WORKORDER_NO");
						String svc_id = rs.getString("svc_id");
						String svc_accept_department = rs.getString("svc_accept_department");
						String svc_accepter = rs.getString("svc_accepter");
						String svc_accept_time = rs.getString("svc_accept_time");
						String resolve_state = rs.getString("resolve_state");
						String REMARK = rs.getString("REMARK");

						sb.append(" <FULL_FAULT_NO>").append(WORKORDER_NO).append("</FULL_FAULT_NO>").append("\n");
						sb.append(" <SYSTEM_PLATFORM>").append(svc_id).append("</SYSTEM_PLATFORM>").append("\n");
						sb.append(" <DEPARTMENT>").append(svc_accept_department).append("</DEPARTMENT>").append("\n");
						sb.append(" <USER>").append(svc_accepter).append("</USER>").append("\n");
						sb.append(" <TESTAUDITTIME>").append(svc_accept_time).append("</TESTAUDITTIME>").append("\n");
						sb.append(" <RESULT_INFO>").append(resolve_state).append("</RESULT_INFO>").append("\n");
						sb.append(" <REMARK>").append(REMARK).append("</REMARK>").append("\n");
					...
					boolean resultFlag = wsClient.outFileDataSHFault(info.wsURL, sb.toString());
					if (!resultFlag) {

						resultFlag = wsClient.outFileDataSHFault(info.wsURL, sb.toString());
						LOG.debug(task.getId() + ", 访问 webservice : " + info.wsURL + ",重试结果" + resultFlag);
					}
